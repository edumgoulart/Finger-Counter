...
class HandTracking:
        ...
        try:
            ...
            if self.results.multi_hand_landmarks:
                ...
                for idx, hand_landmarks in enumerate(self.results.multi_hand_landmarks):
                    # If we have more hands detected than requested IDs, stop or handle accordingly
                    # Here we just map the first N detected hands to the requested IDs
                    if idx >= len(hand_id_list):
                        break
                    
                    current_hand_id = str(hand_id_list[idx])
                    return_dict[current_hand_id] = []
                    ...
                    try:
                        for index_, lm in enumerate(hand_landmarks.landmark):
                            ...
                            # Check if the landmark is present in the index list
                            # Append the x,y coordinates  of the landmark
                            return_dict[current_hand_id].append((cx, cy, lm.z))
                            if show_landmarks:
                                self.mpDraw.draw_landmarks(
                                    img_bgr,
                                    hand_landmarks,
    ...
    def is_finger_up(self,
        ...
        try:
            ...
            lms_dict = self.find_all_landmarks(img_bgr, hand_id_list=hand_id_list)
...
if __name__ == '__main__':
    ...
    for video in video_list:
        main(show_fps=True, video_src="../gallery/{}.mp4".format(video))
